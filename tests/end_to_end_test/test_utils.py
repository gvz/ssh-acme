...
def generate_ca_key(CA):
    ...
    CA.succeed("ssh-keygen -t ed25519 -f /etc/ssh_ca/ca_key -C CA_KEY -N \"\" ")
    ...
    CA.succeed("ssh-keygen -s /etc/ssh_ca/ca_key -h -I CA -n ca,CA -V +1d /etc/ssh/ssh_host_ed25519_key.pub")
    CA.copy_from_vm("/etc/ssh_ca/ca_key.pub", "./ca_key.pub")
...
def prepare_Client(Client, CA):
    ...
    copy_between_hosts(CA,"/etc/ssh_ca/ca_key.pub", Client,"/tmp/ca_key.pub")
...
def build_test_host_config(TestHost, CA, host_key_path="/etc/ssh/ssh_host_ed25519_key"):
    ...
    template = CA.succeed("cat /etc/ssh_ca/hosts/testhost.toml.j2")
    ...
    with open("testhost.toml","w+") as template_file:
      ...
      template_file.close()
    CA.succeed("rm /etc/ssh_ca/hosts/testhost.toml.j2")
    CA.copy_from_host("testhost.toml", "/etc/ssh_ca/hosts/testhost.toml")
    conf = CA.succeed("cat /etc/ssh_ca/hosts/testhost.toml")
    print(conf)

def add_ca_to_knownhost(TestHost, CA):
    pubkey = CA.succeed("cat /etc/ssh_ca/ca_key.pub")
